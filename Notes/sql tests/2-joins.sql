...
SELECT p.PlaylistId, p.Name, COUNT(p.PlaylistId) AS "Num of Songs" FROM PLayList AS p
	LEFT JOIN PlayListTrack AS pt ON p.PlaylistId = pt.PlaylistId
GROUP BY p.PlaylistId, p.Name
ORDER BY COUNT(*) DESC;

-- 4. which sales agent made the most in sales in 2009?
SELECT e.FirstName + ' ' + e.LastName AS "Sales Agent", SUM(i.Total) AS "Sales Total" FROM Invoice AS i
	INNER JOIN Customer AS c ON i.CustomerId = c.CustomerId
	LEFT JOIN Employee AS e ON c.SupportRepId = e.EmployeeId
WHERE i.InvoiceDate >= '2009' AND i.InvoiceDate < '2010'
GROUP BY e.FirstName, e.LastName
ORDER BY SUM(i.Total) DESC;

-- 5. how many customers are assigned to each sales agent?
SELECT e.FirstName + ' ' + e.LastName AS "Sales Agent", COUNT(c.SupportRepId) AS "Num of Customers" FROM Customer AS c
